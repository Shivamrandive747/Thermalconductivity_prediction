...
\SI{87}{\percent}. The correction therefore centres the population rather than merely shrinking it
towards the data. The accuracy figures below are scored under a stricter arrangement, with the
...
\begin{figure}[tb]
  ...
  Shading marks a factor of two. Points are per compound per \SI{100}{\kelvin} bin rather than per
  measurement row, since a few heavily sampled compounds (\ce{TiNiSn} \num{753} rows,
  \ce{ZrNiSn} \num{644}) would otherwise dominate.}
  \label{fig:offset}
\end{figure}
...
Table~\ref{tab:blind} gives the central result. Across \num{38} half Heuslers spanning \num{18}
...
without the correction gives \SI{50.5}{\percent} and \SI{65.8}{\percent}. The bias deserves
separate mention: the median ratio of predicted to measured conductivity is \num{1.00}, against
\num{1.32} uncorrected, so the method is centred and not merely closer. Under the tighter criterion
...
in Table~\ref{tab:blind} and closer to the measurement for \SI{71}{\percent} of compounds. A paired
...
channel, and note that the screens were fixed before the validation set was scored; a reader who
weighs it differently has both numbers. The rank correlation is the more robust claim---Spearman's
...
\num{31} in-domain compounds whose family has at least two other measured members, against
\SI{31.0}{\percent} for our method at the same seed. Head to head on those \num{31} the difference
is not significant (paired Wilcoxon $p=\num{0.22}$) and our method is the closer of the two for only
\SI{42}{\percent} of them. \textbf{On absolute accuracy we do not beat it.}
...
\ce{Ni}--\ce{Sb} (\SI{29.2}{\percent}) are predicted well, while \ce{Bi}--\ce{Pd} is not, at
\SI{60.0}{\percent} with only \SI{33}{\percent} of members within a factor of two and a median bias
of \num{0.40}---a systematic \num{2.5}-fold under-prediction. We issue no predictions in
\ce{Bi}--\ce{Pd} for that reason. This grouping is a \emph{post hoc} description of where errors
...
\SI{75}{\percent} inside ($n=\num{8}$). The separation is carried by the within-$2\times$ rate
...
depend on the new measurement. \ce{Ni}--\ce{Bi} is the most economical, releasing nine predictions
...
dependence is consistent with boundary-limited transport in the Callaway
sense~\cite{callaway1959model}, and we describe it that way, but the Cauchy--Schwarz argument of
