...
The cap is not a formality, and anyone applying Eq.~\eqref{eq:transfer} should know where it binds.
With these constants $c\,(T/300)^{p}$ reaches unity at $T \approx \SI{696}{\kelvin}$, so above about
\SI{700}{\kelvin} the transfer function is the identity and no correction is applied at all. That
regime is not a corner of the data: \num{138} of the \num{333} half-Heusler rows in the blind test
lie above it, two in five. The reported accuracy therefore averages a corrected low-temperature
regime with an uncorrected high-temperature one, and $p$ is in effect fitted on the sub-cap rows.
Section~\ref{sec:limitations} returns to this, since the compounds where the calculation falls
\emph{below} the measurement---which no downward correction can reach---concentrate in the same
high-temperature regime. Both are fitted quantities
...
\begin{table*}[tp]
  ...
  % 0.56 of the line width, plus the count and name columns, exceeded the text block. 0.46 fits.
